...
class AIServiceTester {
  ...
  private async testAIHealth(): Promise<void> {
    try {
      const response = await axios.get(`${API_BASE}/ai/health`, { timeout: 10000 });
  ...
  private async testN8nStatus(): Promise<void> {
    try {
      const response = await axios.get(`${API_BASE}/n8n/status`, { timeout: 10000 });
  ...
  private async testAITextGeneration(): Promise<void> {
    try {
      const response = await axios.post(`${API_BASE}/ai/generate-text`, {
        prompt: '请简单介绍一下古诗《春晓》',
        options: {
          temperature: 0.7,
          max_tokens: 200
  ...
  private async testPoetryContentGeneration(): Promise<void> {
    try {
      const response = await axios.post(`${API_BASE}/ai/generate-poetry-content`, {
        type: 'hint',
        poem: {
          title: '春晓',
          author: '孟浩然',
  ...
  private async testLearningAdvice(): Promise<void> {
    try {
      const response = await axios.post(`${API_BASE}/ai/generate-learning-advice`, {
        userProgress: {
          learned_count: 3,
          accuracy: 85,
